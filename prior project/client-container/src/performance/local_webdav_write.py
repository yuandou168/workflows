...
for count in range(run_count):
    ...
    log_event("Creating random byte array " + file_size_name)
    file_bytes = create_bytes(file_size)

    file_path = "/tmp/files/" + file_size_name
...
write_json(file_name="local_webdav_write_" + file_size_name, json_data=runs)